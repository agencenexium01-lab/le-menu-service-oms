...
export default function ChefPointOrderDetailPage() {
  ...
  // States for verification/validation representing physical or card payments
  ...
  const fetchOrderDetails = async () => {
    ...
    try {
      ...
      // Specific branch security guard verification on the fetched order
      if (userProfile && userProfile.role === 'chef_point') {
        const hasAccess = orderData.branchId === userProfile.branchId || orderData.assignedBranchId === userProfile.branchId;
        if (!hasAccess) {
          setError("Vous n'êtes pas autorisé à inspecter une commande rattachée à une autre succursale.");
          setLoading(false);
          return;
        }
      }
      ...
      console.error('Erreur chargement détails commande chef point:', err);
  ...
  const handleSaveNotes = async () => {
    ...
    try {
      ...
      setTimeout(() => setNotesSuccess(false), 4500);
    } catch (err: any) {
    ...
    return (
      <div className="p-6 bg-red-500/10 border border-red-500/20 rounded-2xl max-w-lg mx-auto space-y-4 mt-10">
        <div className="flex items-center gap-3 text-red-400">
          ...
          <h3 className="font-extrabold text-white">Accès Rejeté</h3>
        </div>
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          <Link 
            to={`/pro/point/${branchId}/commandes`} 
            className="inline-flex items-center gap-2 text-xs font-bold text-slate-400 hover:text-white bg-white/5 border border-white/5 hover:border-white/10 px-3.5 py-2 rounded-xl transition-all no-print"
          >
            <ArrowLeft className="w-4 h-4" />
            <span>Registre du Point</span>
          </Link>

          {/* Print button on Chef/Admin side */}
          <button
            onClick={() => window.print()}
            className="inline-flex items-center gap-2 text-xs font-bold text-slate-300 hover:text-white bg-white/5 border border-white/5 hover:border-white/10 px-3.5 py-2 rounded-xl transition-all cursor-pointer print-button no-print"
          >
        ...
        {canCreateQuote && (
          <Link
            to={`/pro/point/${branchId}/commandes/${order.id}/quote`}
            className="inline-flex items-center gap-2 px-5 py-2.5 bg-gradient-to-r from-yellow-500 to-amber-500 hover:from-yellow-400 hover:to-amber-400 text-slate-950 font-black rounded-xl text-xs shadow-[0_4px_15px_rgba(234,179,8,0.3)] transition-all no-print"
          >
      ...
      <div className="backdrop-blur-xl bg-[#111A36]/60 border border-white/10 rounded-3xl p-6 md:p-8 space-y-6 relative overflow-hidden">
        ...
        <div className="flex flex-col md:flex-row md:items-start justify-between gap-6">
          <div className="space-y-3">
            ...
            <h1 className="text-2xl md:text-3xl font-black text-white tracking-tight">
              Fiche de Contrôle Technique — Succursale
            </h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="backdrop-blur-xl bg-[#111A36]/40 border border-white/10 rounded-3xl p-6 md:p-8 space-y-4">
        ...
        <div className="space-y-6">
          ...
          {/* BLOCK PAIEMENT */}
          <div className="backdrop-blur-xl bg-[#111A36]/40 border border-white/10 rounded-3xl p-6 space-y-4">
            <div className="flex items-center justify-between border-b border-white/5 pb-3">
              <h4 className="font-extrabold text-white text-sm flex items-center gap-2">
                <Coins className="w-4 h-4 text-emerald-400" />
                <span>Règlement & Transaction</span>
              </h4>
              {order.paymentStatus === 'paid' ? (
                <span className="px-2.5 py-0.5 rounded text-[10px] font-black border uppercase bg-emerald-500/10 border-emerald-500/20 text-emerald-400">
                  ✅ Payé
                </span>
              ) : (
                <span className="px-2.5 py-0.5 rounded text-[10px] font-black border uppercase bg-amber-500/10 border-amber-500/20 text-amber-550">
                  ⚠️ Non payé
                </span>
              )}
            </div>
            ...
            {order.paymentStatus === 'paid' ? (
              <div className="text-xs text-slate-300 space-y-1 bg-emerald-500/5 border border-emerald-500/10 p-3.5 rounded-2xl">
                <p className="font-bold text-emerald-400">✓ Paiement validé au comptoir</p>
                {order.paidBy && (
                  <div className="text-[10px] text-slate-400 pt-1 space-y-0.5">
                    <p>Opérateur : <span className="font-extrabold text-white">{order.paidByName || order.paidBy}</span></p>
                    <p>Le : <span className="font-semibold text-slate-300">{getFormattedDate(order.paidAt)}</span></p>
                  </div>
                )}
              </div>
            ) : (
              <div className="space-y-3">
                <p className="text-[11px] text-slate-400 leading-normal">
                  Le règlement de cette commande est actuellement en attente. Une fois acquitté au comptoir physique, veuillez impérativement régulariser ci-dessous.
                </p>
                ...
                {(order.status === 'pret' || order.status === 'livre') && (
                  <button
                    ...
                    className="w-full py-2.5 bg-emerald-600 hover:bg-emerald-500 text-white font-extrabold rounded-xl text-xs flex items-center justify-center gap-2 cursor-pointer transition-all border border-emerald-500/25 shadow-lg shadow-emerald-950/20 active:scale-95"
                  >
      ...
      {showPaymentModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/80 backdrop-blur-sm p-4 animate-fade-in" id="payment-confirmation-modal">
          <div className="bg-[#111A36] border border-white/10 rounded-3xl max-w-md w-full p-6 space-y-6 shadow-2xl relative">
            <div className="text-center space-y-3">
              <div className="w-12 h-12 bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 rounded-full flex items-center justify-center mx-auto shadow-lg">
                <Coins className="w-6 h-6" />
              </div>
              <h3 className="text-lg font-black text-white">Confirmer le règlement</h3>
              <p className="text-xs text-slate-300 leading-relaxed">
                Confirmer que la commande <span className="text-yellow-400 font-extrabold">#{order?.orderNumber}</span> a bien été réglée en physique au comptoir ?
              </p>
            </div>
            ...
            <div className="flex gap-3 pt-2">
              <button
                ...
                className="flex-1 py-2.5 bg-white/5 border border-white/10 hover:bg-white/10 text-slate-350 rounded-xl text-xs font-bold transition-all cursor-pointer"
              >
                Annuler
              </button>
              <button
                type="button"
                onClick={async () => {
                  if (!order?.id || !userProfile) return;
                ...
                disabled={validatingPayment}
                className="flex-1 py-2.5 bg-emerald-600 hover:bg-emerald-500 disabled:bg-slate-800 text-white rounded-xl text-xs font-black transition-all flex items-center justify-center gap-1.5 shadow-lg shadow-emerald-950/30 cursor-pointer"
              >
                {validatingPayment ? 'Validation...' : 'Confirmer'}
              </button>
            </div>
          </div>
        </div>
